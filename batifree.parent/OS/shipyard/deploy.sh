...
IMAGE=${IMAGE:-janeczku/shipyard-armv7:latest}
...
start_discovery() {
    ...
    ID=$(docker run \
        ...
        janeczku/etcd-armv7:latest -addr $SHIPYARD_IP:$DISCOVERY_PORT -peer-addr $SHIPYARD_IP:$DISCOVERY_PEER_PORT)
}
...
wait_for_available() {
    ...
    until $(docker run --rm janeczku/curl-armv7 --output /dev/null --connect-timeout 1 --silent --head --fail $TLS_OPTS $SHIPYARD_PROTOCOL://$IP:$PORT/ > /dev/null 2>&1); do
        printf '.'
        sleep 1 
    done
    printf '\n'
