...
public abstract class AffaireManagerImpl<IT extends IAffaire> extends BfGenericManagerImpl<IT, Integer> implements IAffaireManager<IT> {
	...
	@Override
	public <AFF extends IAffaire> AFF getClone(IT pAffaireOrigin, AFF pNewAffaireDest) throws BatifreeException {
	...
	@Override
	public IT save(IT pAffaire) throws BatifreeException {
		...
		if (pAffaire.isListEtudeChanged()) {
			...
			// Save des Etudes
			IEtudeManager etudeMng = ApplicationBf.getApplication().getEtudeManager();
		...
		if (pAffaire.isListEcheancierChanged()) {
			...
			// Save des Echeancier
			IEcheancierManager echeancierMng = ApplicationBf.getApplication().getEcheancierManager();
